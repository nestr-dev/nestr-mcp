...
export const schemas = {
  ...
  bindConnector: z.object({
    ...
    ownerType: z.enum(["role", "role-domain", "workspace", "user"]).describe("Who gets access. 'role' is usually what you want: pass a role nest ID and the connector's domain is found or created under it. 'role-domain' targets an existing domain directly. 'workspace' gives everyone. 'user' is for one person's own account, which is the only case where a role binding would be wrong: see the tool description. 'agent' is deliberately not available here."),
    ownerId: z.string().describe("Owner ID. role: the role nest ID. role-domain: the domain nest ID. workspace: the workspace ID. user: the person's user ID."),
  }),
